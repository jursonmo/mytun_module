...
#define MYTUN_MINOR 201
#define DRV_NAME	"mytun"
...
static bool put_to_ringbuf(struct tun_struct *tun,
			 struct tun_file *tfile,
			    struct sk_buff *skb, 
			    rbf_t *rbf);
void notify_init(struct rbf_notify *notify) 
{
			    ...
			    struct iov_iter *iter)
{
	...
	KUMAP_DEBUG("skb->len=%u, data-head=%lu, mac_header=%u, network_header=%u, transport_header=%u \n", skb->len, skb->data-skb->head, skb->mac_header, skb->network_header,skb->transport_header);
	KUMAP_DEBUG("11111 skb->len=%u, vlan_offset=%d, iter->type=0x%x, iov_offset=%lu,count=%lu, nr_segs=%lu, iov->iov_len=%lu", skb->len,  vlan_offset, iter->type, iter->iov_offset,iter->count, iter->nr_segs,iter->iov->iov_len);
	...
	KUMAP_DEBUG("22222 skb->len=%u, vlan_offset=%d, iter->type=0x%x,iov_offset=%lu,count=%lu, nr_segs=%lu, iov->iov_len=%lu ", skb->len,  vlan_offset, iter->type, iter->iov_offset,iter->count, iter->nr_segs,iter->iov->iov_len);
done:
			    ...
			    rbf_t *rbf)
{
	...
	struct iov_iter iter;
	struct  kvec kv;
	if (rbf_node_size(rbf) < skb->len){
		KUMAP_ERROR("rbf_node_size=%u, skb->len=%u", rbf_node_size(rbf) , skb->len);
		return false;
	}
	spin_lock(&tfile->rbf_lock);
	buf = (char *)rbf_get_buff(rbf);
	if (!buf) {
	...
	if (vlan_hlen) {
		...
		memcpy(buf, skb->data, vlan_offset); 
		//copy vlan hdr
		memcpy(buf+vlan_offset , &veth, sizeof(veth));
		buf = buf + vlan_offset + sizeof(veth);		
	}
	*/
	iter.type = ITER_KVEC;
...
{
	//can not be here, socket.c setsockopt SYSCALL_DEFINE5 --> sockfd_lookup_light --> sock_from_file will check if (file->f_op == &socket_file_ops)
	if (sock)
		printk("have sock \n");
...
{	
	...
	if (!addr)
		return -EFAULT;
	tfile->rbf = rbf_init(addr, PAGE_SIZE*(1 <<(order-1)), req.packet_size);
	if (!tfile->rbf)
		return -EFAULT;
	...
	tfile->tx_rbf = rbf_init((void *)rbf_tail(tfile->rbf) , PAGE_SIZE*(1 <<(order-1)), req.packet_size);
	if (!tfile->tx_rbf) {
		goto free_rbf;
	}
	rbf_dump(tfile->tx_rbf);
	
	printk("tail: %p, addr + half =%p , tx_rbf=%p====\n", rbf_tail(tfile->rbf), (char *)addr + PAGE_SIZE*(1 <<(order-1)), tfile->tx_rbf);	
	//check 
	if (rbf_size(tfile->rbf) !=rbf_size(tfile->tx_rbf) || rbf_tail(tfile->rbf) != (void*)tfile->tx_rbf){
		printk("check fail ==========\n");
		goto free_rbf;
	}
	tfile->mmap_buf_size = PAGE_SIZE*(1 <<order);
	return 0;
	
free_rbf:
	free_pages((unsigned long)tfile->rbf, get_order(rbf_size(tfile->rbf))+1);
	tfile->rbf = NULL;
err_out:
	return -EFAULT;
...
static long mytun_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	int res = 0;
	...
	switch(cmd) {
		case TUN_IOC_SEM_WAIT:
			...
			res = down_interruptible(&notify->sem);
			if(res) {
...
static int tun_chr_close(struct inode *inode, struct file *file)
{
	...
	if (tfile->tx_rbf) {		
		free_pages((unsigned long)tfile->tx_rbf, get_order(rbf_size(tfile->tx_rbf)));
		tfile->tx_rbf = NULL;
	}
...
/*
	tun_chr_mmap add by mo ,  mmap data for user application
	higher performence , less system call
*/
static int tun_chr_mmap(struct file *file, struct vm_area_struct *vma)
{
	int i, res;
	...
	if ((mapped_size + vma->vm_pgoff * PAGE_SIZE) > tfile->mmap_buf_size) {
		printk("shm: shm get mem size failed.\n"
				"mapsize: %ld, pgoff: %lu, memsize: %ldk.\n",
				(unsigned long)mapped_size, (unsigned long)vma->vm_pgoff, (unsigned long)rbf_size(tfile->rbf));
...
struct socket *mytun_get_socket(struct file *file)
{
	struct tun_file *tfile;
...
EXPORT_SYMBOL_GPL(mytun_get_socket);
...
MODULE_LICENSE("GPL");
MODULE_ALIAS_MISCDEV(MYTUN_MINOR);
MODULE_ALIAS("devname:net/mytun");
